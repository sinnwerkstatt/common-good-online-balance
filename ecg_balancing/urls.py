...
urlpatterns = patterns('',
	...
	url(r'^company/(?P<company_slug>[\w-]*)/(?P<balance_year>[0-9]*)/(?P<indicator_id>.*)$', login_required(CompanyBalanceIndicatorDetailView.as_view()), name='indicator-detail'),
	url(r'^company/(?P<company_slug>[\w-]*)/(?P<balance_year>[0-9]*)/(?P<indicator_id>.*)/update$', login_required(CompanyBalanceIndicatorUpdateView.as_view()), name='indicator-update'),
