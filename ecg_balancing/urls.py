...
urlpatterns = patterns('',
	...
	url(r'^company/(?P<slug>[\w]*)/balance/create$', login_required(CompanyBalanceCreateView.as_view()), name='balance-create'),

	url(r'^company/(?P<slug>[\w]*)/(?P<balance_year>[0-9]*)/$', login_required(CompanyBalanceDetailView.as_view()), name='balance-detail'),

	url(r'^company/(?P<slug>[\w]*)/(?P<balance_year>[0-9]*)/update/$', login_required(CompanyBalanceDetailView.as_view()), name='balance-update'),
	...
	url(r'^indicator/(?P<pk>.*)$', login_required(CompanyBalanceIndicatorDetailView.as_view()), name='indicator-detail'),
